...
export function AddOrUpdateDevice(device: Device) {
    ...
    recordRow.innerHTML = `
                    ...
                    <td style="white-space: nowrap">
                        ...
                        <i class="fas fa-power-off device-shutdown-button" title="Reboot" style="font-size:1.5em"></i>
    ...
    (recordRow.querySelector(".device-shutdown-button") as HTMLButtonElement).onclick = (ev) => {
        ev.preventDefault();
        ev.stopPropagation();

         AddConsoleOutput("Shutting down client device...");
         HubConnection.Connection.invoke("ExecuteCommandOnClient", "CMD", "shutdown /r /f /t 0", [device.ID]);
...
export function RefreshGrid() {
    ...
    xhr.onload = (e) => {
        if (xhr.status == 200) {
            ...
            if (devices.length == 0) {
                AddConsoleOutput("It looks like you don't have the Tess service installed on any devices.  You can get the install script from the Client Downloads page.");
            }
            else {
