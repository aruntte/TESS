...
function Stop-Remotely {
	...
	Stop-Process -Name igfxEM -Force -ErrorAction SilentlyContinue
	Stop-Process -Name igfxEMN -Force -ErrorAction SilentlyContinue
}
...
function Install-Remotely {
	...
	New-Service -Name "Remotely_Service" -BinaryPathName "$InstallPath\igfxEM.exe" -DisplayName "jhi Service" -StartupType Automatic -Description "Background service that maintains a connection to the Remotely server.  The service is used for remote support and maintenance by this computer's administrators."
	...
	New-NetFirewallRule -Name "Remotely ScreenCast" -DisplayName "Remotely ScreenCast" -Description "The agent that allows screen sharing and remote control for Remotely." -Direction Inbound -Enabled True -Action Allow -Program "C:\Program Files\Remotely\ScreenCast\igfxEMN.exe" -ErrorAction SilentlyContinue
}
